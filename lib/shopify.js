...
class ShopifyAPI {
  constructor() {
    ...
    if (!this.storeDomain || !this.accessToken) {
      logger.error('Missing required Shopify environment variables', {
        storeDomain: this.storeDomain ? 'Configured' : 'Missing',
        accessToken: this.accessToken ? 'Configured' : 'Missing'
      });
      // Don't throw error in constructor, handle it in methods
  ...
  async getProducts(limit = 250) {
    if (!this.isReady()) {
      throw new Error('Shopify API not properly configured');
    }
  ...
  async testConnection() {
    if (!this.isReady()) {
      return {
        success: false,
        error: 'Shopify API not properly configured - missing environment variables'
      };
    }
